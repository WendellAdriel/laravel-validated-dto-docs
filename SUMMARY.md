# Table of contents

* [✅ Validated DTO for Laravel](README.md)

## 🚀 Getting Started

* [Installation](getting-started/installation.md)
* [Configuration](getting-started/configuration.md)
* [Upgrade Guide](getting-started/upgrade-guide.md)
* [Changelog](getting-started/changelog.md)

## 😎 The Basics <a href="#basics" id="basics"></a>

* [Generating DTOs](basics/generating-dtos.md)
* [Defining DTO Properties](basics/defining-dto-properties.md)
* [Defining Validation Rules](basics/defining-validation-rules.md)
* [Creating DTO Instances](basics/creating-dto-instances.md)
* [Accessing DTO Data](basics/accessing-dto-data.md)
* [Defining Default Values](basics/defining-default-values.md)
* [Transforming DTO Data](basics/transforming-dto-data.md)
* [Mapping DTO properties](basics/mapping-dto-properties.md)
* [Simple DTOs](basics/simple-dtos.md)
* [Resource DTOs](basics/resource-dtos.md)
* [Wireable DTOs](basics/wireable-dtos.md)
* [Lazy Validation](basics/lazy-validation.md)
* [Generating TypeScript Definitions](basics/generating-typescript-definitions.md)

## 🎨 Customize

* [Custom Error Messages and Attributes](customize/custom-error-messages-and-attributes.md)
* [Custom Exceptions](customize/custom-exceptions.md)

## 🧪 Type Casting

* [Introduction](type-casting/introduction.md)
* [Available Types](type-casting/available-types.md)
* [Create Your Own Type Cast](type-casting/create-your-own-type-cast.md)
* [Casting Eloquent Model properties to DTOs](type-casting/casting-eloquent-model-properties-to-dtos.md)
